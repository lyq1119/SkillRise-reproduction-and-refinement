#!/usr/bin/env bash
set -x
# SkillRise cross-task meta-RL on ScienceWorld (K=3 tasks/group, N=8 trials/group).
# Solve uses pure terminal score/100 reward; Curate gets gamma-discounted downstream.
ENGINE=${1:-vllm}
...
pip3 install scienceworld gym

project_name="verl_agent_sciworld_meta"
TIMESTAMP="$(date +%Y%m%d%H%M%S)"
experiment_name="skillrise_sciworld_qwen3-4b_K3_${TIMESTAMP}"
...
GROUP_FILE="${SCRIPT_DIR}/../../data/groups/skillrise_sciworld_K3.jsonl"
...
python3 -m verl.trainer.main_ppo \
    ...
    env.env_name="skillrise_sciworld" \
    env.seed=0 \
    env.max_steps=30 \
    env.max_turns=30 \
    env.rollout.n=$group_size \
    env.num_attempts=$K \
    +env.simplifications_preset=easy \
    env.resources_per_worker.num_cpus=$num_cpus_per_env_worker \
    env.max_env_per_rollout=128 \
    ...
    "$@" \
    2>&1 | tee "${OUTPUT_DIR}/skillrise_sciworld_qwen3_4b.log"
