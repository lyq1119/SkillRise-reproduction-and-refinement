#!/usr/bin/env bash
set -x
# DEBUG: SkillRise cross-task meta-RL on ScienceWorld, K=3, 2 steps, single GPU.
ENGINE=${1:-vllm}
...
pip3 install scienceworld gym >/dev/null 2>&1 || true

train_data_size=2         # GROUPS per step
val_data_size=2
group_size=2              # N trials per group (env.rollout.n)
...
python3 -m verl.trainer.main_ppo \
    ...
    trainer.project_name='sciworld_skillrise_debug' \
