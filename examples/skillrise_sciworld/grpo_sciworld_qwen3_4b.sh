#!/usr/bin/env bash
set -x
# Pure GRPO baseline on ScienceWorld (1 attempt, no reflection).
# Hyperparameters aligned with the alfworld GRPO baseline; env shape (16x24)
# matches BEACON's per-step rollout volume.
ENGINE=${1:-vllm}
...
experiment_name="grpo_sciworld_qwen3-4b_${TIMESTAMP}"
...
python3 -m verl.trainer.main_ppo \
    ...
    "$@" \
    2>&1 | tee "${OUTPUT_DIR}/grpo_sciworld_qwen3_4b.log"
