set -x
# DEBUG: pure GRPO baseline (1 attempt, no meta), short run (2 steps), single GPU.
# group_size kept small for the 18-CPU debug node (formal run uses 24 on an 8-GPU node).
ENGINE=${1:-vllm}
...
max_env_per_rollout=48   # 8x24=192 > 48 -> 2 groups/chunk = 48 workers, 4 chunks (tests chunked rollout)
...
python3 -m verl.trainer.main_ppo \
    ...
    env.max_env_per_rollout=$max_env_per_rollout \
