...
def compute_skillrise_outcome_advantage(token_level_rewards: torch.Tensor,
                                    response_mask: torch.Tensor,
                                    index: np.array,
                                    traj_index: np.array,
                                    task_pos: np.array,
                                    role: np.array,
                                    epsilon: float = 1e-6,
                                    curate_loss_weight: float = 1.0,
                                    mode: str = "mean_norm",
                                    ):
    """Group-relative advantage for cross-task meta-RL (SkillRise).

    Each token's scalar score = sum of token_level_rewards over the response.
    Group key = (group_uid, task_pos, role): the N trials of the same group,
    at the same task position, in the same role (solve/curate) form one group.
    Within a group, every trajectory contributes ONE scalar (dedup by traj_uid)
    so multi-turn solve tokens are not double-counted in the mean.

    Solve and curate tokens never mix (role is part of the key). Curate-token
    advantages are scaled by curate_loss_weight.
    """
    ...
    with torch.no_grad():
        bsz = scores.shape[0]
        keys = [(index[i], to_hashable(task_pos[i]), role[i]) for i in range(bsz)]
        for i in range(bsz):
            dedup = (keys[i], traj_index[i])
            if dedup in seen_pairs:
                continue
            seen_pairs.add(dedup)
            id2score[keys[i]].append(scores[i])
        ...
        for k in id2score:
            if len(id2score[k]) == 1:
                ...
                vals = torch.tensor(id2score[k])
                id2mean[k] = torch.mean(vals)
                id2std[k] = torch.std(vals)

        out = scores.clone()
        for i in range(bsz):
            k = keys[i]
            if remove_std:
                out[i] = scores[i] - id2mean[k]
            else:
                out[i] = (scores[i] - id2mean[k]) / (id2std[k] + epsilon)
            if role[i] == 'curate':
                out[i] = out[i] * curate_loss_weight

        advantages = out.unsqueeze(-1).tile([1, response_length]) * response_mask
