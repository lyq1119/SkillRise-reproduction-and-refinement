...
def compute_groups_per_chunk(train_batch, group_n, max_env_per_rollout):
    """Largest number of groups g such that g divides train_batch and
    g*group_n <= max_env_per_rollout. Ensures equal chunks and a fixed env pool.

    Returns train_batch (no chunking) if max_env_per_rollout <= 0 or the full
    batch already fits.
    """
    if max_env_per_rollout is None or max_env_per_rollout <= 0:
        return train_batch
    if train_batch * group_n <= max_env_per_rollout:
        return train_batch
    cap = max(1, max_env_per_rollout // group_n)
    for g in range(min(cap, train_batch), 0, -1):
        if train_batch % g == 0:
            return g
    return 1
