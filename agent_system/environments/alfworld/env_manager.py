...
def make_envs(config):
    ...
    if "alfworld" in config.env.env_name.lower():
        ...
        # Configurable per-env-worker Ray resources (default 0.5 = original).
        rpw = config.env.get('resources_per_worker', None)
        if rpw is not None:
            resources_per_worker = {'num_cpus': rpw.get('num_cpus', 0.5)}
        ...
        # max_env_per_rollout (the rollout loop splits the batch into chunks that
        # each reuse this pool). val pool is small (group_n=1) and not capped.
        ...
        _envs = build_alfworld_envs(alf_config_path, config.env.seed, groups_per_chunk, group_n, is_train=True, env_kwargs=env_kwargs, resources_per_worker=resources_per_worker)
