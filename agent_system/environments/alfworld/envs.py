...
class AlfworldEnvs(gym.Env):
    def __init__(self, alf_config_path, seed=0, env_num=1, group_n=1, is_train=True, env_kwargs={},
        ...
        # Configurable per-worker resources (default 0.5 preserves original behavior).
        # Lower this when env_num*group_n is large so the env actors don't consume
        # every CPU and deadlock the trainer's GPU worker group.
        if resources_per_worker is None:
            resources_per_worker = {'num_cpus': 0.5}
        worker_cls = AlfworldWorker.options(**resources_per_worker)
