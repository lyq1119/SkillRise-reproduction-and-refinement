...
class WebshopEnvironmentManager(EnvironmentManagerBase):
    def __init__(self, envs, projection_f, num_attempts, do_reflection, config):        
        ...
        ## curr_traj_idx is used to track the current trajectory index for SkillRise
...
def make_envs(config, val_only=False):
    ...
    if "webshop" in config.env.env_name.lower():
        ...
        # Configurable per-env-worker Ray resources (default falls back to the
        # WebshopWorker class default of num_cpus=0.1, i.e. original behaviour).
        rpw = config.env.get('resources_per_worker', None)
        if rpw is not None:
            resources_per_worker = {'num_cpus': rpw.get('num_cpus', 0.1)}
        ...
        # max_env_per_rollout. The rollout loop then splits the batch into chunks that
        # each reuse this pool. Backward compatible: when the full batch already fits
        # (e.g. LaMer 16x8=128 <= 128) groups_per_chunk == train_batch -> single chunk,
        # identical to the original path. GRPO (16x24=384 > 128) chunks to 3x128.
        ...
        _envs = build_webshop_envs(seed=config.env.seed, env_num=groups_per_chunk, group_n=group_n, is_train=True, env_kwargs=env_kwargs, resources_per_worker=resources_per_worker)
        ...
        import time
        time.sleep((groups_per_chunk * group_n + config.data.val_batch_size) * 0.1) # wait for the envs to be ready
