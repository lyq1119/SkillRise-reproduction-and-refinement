...
class WebshopMultiProcessEnv(gym.Env):
    ...
    def __init__(
        ...
        # starving the trainer's GPU worker group). Falls back to the class default
        # (num_cpus=0.1) when not provided, preserving the original behaviour.
        if resources_per_worker is not None:
            worker_cls = WebshopWorker.options(**resources_per_worker)
        else:
            worker_cls = WebshopWorker
        self._workers = []

        for i in range(self.num_processes):
            worker = worker_cls.remote(seed + (i // self.group_n), self._env_kwargs)
