...
def parse_reflection(traj_idx, past_traj, reflection, reflection_type='reflection_only'):
    if traj_idx == 0:
        ...
        for _idx in range(traj_idx):
            # Use .get(): when do_reflection=False (e.g. GRPO val replays the SAME task for
            # pass@k WITHOUT a reflect phase), self.reflections stays empty -> reflection[_idx]
            # would KeyError. Missing reflection/history just renders as empty.
