...
class EnvironmentManagerBase:
    ...
    def success_evaluator(self, *args, **kwargs) -> Dict[str, np.ndarray]:
        ...
        for bs in range(batch_size):
            ...
            scores = [0.0 for _ in range(self.num_attempts)]
            for i in reversed(range(len(total_batch_list[bs]))):
                ...
                if batch_item['active_masks']:
                    ...
                    if batch_item['phase'] == 'play':
                        ...
                        # Raw 0~1 partial-match score (WebShop) if the env reports it.
                        if info.get('task_score', 0.0):
                            scores[traj_idx] = max(scores[traj_idx], float(info['task_score']))

            _won = False
            _score = 0.0
            for traj_idx, won in enumerate(wons):
                _won = _won or won
                _score = max(_score, scores[traj_idx])
                success[f'success_rate[{traj_idx}]'].append(_won)
                # Cumulative best raw score over attempts (pass@k analogue for Score).
                success[f'task_score[{traj_idx}]'].append(_score)
